...
#     expect(Refinery::VERSION).not_to be nil
#   end

#   it "loads contexts" do
#     expect(UserRefinery).to eq 'something'
#   end
# endf

# require 'rails_helper'

# TODO: refacor all variables into contexts
require 'spec_helper'

RSpec.describe Refinery do
  include_context 'user refinery'
  ...
  let(:refinery_with_multiple_input_values) do
    refinery.permit(%i[month year])
  end
  ...
  describe '.refine' do
    context 'when a non empty hash is passed' do
      it 'returns a hash of refined permit & association values' do
        result = refinery.refine(params)
    ...
    context 'when multiple attributes are defined for a single input' do
      it 'raises ArgumentError with a message' do
        message =
          "UserRefinery recieved multiple attributes for a single input."
        expect { refinery_with_multiple_input_values }.
    ...
    context 'when unhandled keys are supplied' do
      it 'raises UnhandledKeys with a message' do
        expect { refinery.refine(unexpected_params) }.
          to raise_exception(Errors::UnhandledKeys,
            'UserRefinery received unhandled keys: income.')
      end
    end

    context 'when a empty hash is passed' do
      it 'raises ArgumentError with a message' do
        expect { refinery.refine(empty_hash) }.
  ...
  describe '.reduce_handlers' do
    context 'when handlers are present' do
      it 'returns a reduced hash of parsed input values' do
        result = refinery.reduce_handlers(handlers, params)
        expect(result).to eq parsed_values
      end
    end
    ...
    context 'when handlers are not present' do
      it 'returns an empty hash' do
        result = refinery.reduce_handlers(empty_array, params)
  ...
  describe '.handlers_values' do
    context 'when params are defined' do
      it 'returns a array of input handlers' do
        result = refinery.handlers_values
