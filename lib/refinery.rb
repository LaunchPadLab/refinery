...
require "refinery/exception_methods"
require "refinery/assertions"
...
module Refinery
  ...
  def parse_association(name:, value:, options:)
    refinery = find_refinery(name)
    wrapped_values = [value].flatten
    has_many = options[:has_many]

    refined_values = wrapped_values.map do |wrapped_value|
      refinery.refine(wrapped_value)
    end

    if has_many
      { name => refined_values }
    else
      { name => refined_values.pop }
    end
  end
end
